...
var Circuit = new Array(components.length);
/*
for (i = 0; i < components.length; i++) {
  var lines = components[i].split('\n');
  var compType, compRes, compVolt, startx, starty, endx, endy, direction;
  for (j = 0; j < lines.length; j++) {
    var thisline = lines[j].split('"');
    if (thisline[1] == "type") { compType = (thisline[3]); }
    if (thisline[1] == "resistance") { compRes = parseFloat(thisline[3]); }
    if (thisline[1] == "voltageDrop") { compVolt = parseFloat(thisline[3]); }
    if (thisline[1] == "start.x") { startx = parseFloat(thisline[3])/100; }
    if (thisline[1] == "start.y") { starty = parseFloat(thisline[3])/100; }
    if (thisline[1] == "end.x") { endx = parseFloat(thisline[3])/100; }
    if (thisline[1] == "end.y") { endy = parseFloat(thisline[3])/100; }
    if (thisline[1] == "sideID") { direction = parseInt(thisline[3]); } 
  }
  Circuit[i] = new CircuitComponent(compType, compRes, compVolt, startx, starty, endx, endy, direction);
}
*/
...
function CircuitComponent(type, res, volt, startx, starty, endx, endy, direction) {
  this.compType = type;
  this.resistance = res;
  this.voltage = volt;
...
var numComps = components.length;
...
var voltageTotal = resistanceTotal = 0;
for (i = 0; i < Circuit.length; i++) {
  voltageTotal += Circuit[i].voltage;
  resistanceTotal += Circuit[i].resistance;
}
console.log(resistanceTotal, IonNumConst, numWalls);
...
var current = (voltageTotal / resistanceTotal)*currentConst; // currentConst added to make flows still visible
